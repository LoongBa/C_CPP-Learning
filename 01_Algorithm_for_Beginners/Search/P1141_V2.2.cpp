...
// V3.0 : 基于 bfs
// 当前版本为 V2.2：配合邻接表，基于一维数组保存 visited 和 area
#include <iostream>
#include <vector>
#include <utility>
#include <stack>
...
// 基于栈的DFS函数
void dfs(int n, int startIndex, const vector<vector<int>> &adjList, vector<bool> &visited, vector<int> &group)
{
    // 使用队列来实现 BFS
    stack<int> tasks;
...
int main()
{
    ...
    while (m--)
    {
        ...
        if (area[index] == -1)
        {
            ...
            // 从当前位置开始BFS搜索
            dfs(n, index, adjList, visited, group);
