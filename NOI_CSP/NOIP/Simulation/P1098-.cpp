...
int main()
{
    ...
    {
        char dash = line[i]; // 当前字符

        // 仅左右各有 1 个字符时，才能取到窗口，否则不处理
        if (i > 0 && i < line.length() - 1)
        {
            if (dash == '-') // 当前字符是减号
            {
                // 检查是否满足展开条件：都是数字或都是小写字母
                char left = line[i - 1];  // 上个字符（可能为减号）
                char right = line[i + 1]; // 下个字符
                ...
                if (right > left && sameType) // 检查 right 是否大于 left，且两者类型相同
                {
                    ...
                    result += ext;  // 扩展结果代替减号
                    result += right;
                    i += 1;         // 下一轮跳过减号，但右元素作为新窗口左元素
                    continue;       // 跳过当前循环
                }
            }
        }
        result += dash; // 添加当前字符，注意不能在前面加
    }
